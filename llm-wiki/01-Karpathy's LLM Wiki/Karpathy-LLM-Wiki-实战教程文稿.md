# LLM Wiki实战：让AI揪出梁文锋实录的矛盾

> 文中的演示以"投喂几份梁文锋实录文档、提炼核心观点、交叉校验多来源之间是否矛盾"为例，你可以把来源换成任何主题的资料。
>
> **👉 想要 LLM Wiki实战 素材**
>
> **🚀 获取通道：**
...
把访谈、PDF、文章等资料放进 `raw` 文件夹，再让 AI 摄取后，原本散落在不同文档里的信息会被整理成一组**相互链接、可持续更新的知识页面**。打开 Obsidian 的关系图谱，你可以直观看到人物、公司、技术路线、商业化路径、组织文化等主题如何彼此关联；点击任意节点，就能继续追溯具体观点和原始来源。

更实用的是，这不只是一张“看起来很酷”的图。你可以直接让 AI：

- 汇总某个人或某个主题在多份资料中的核心观点
- 对比不同来源对同一问题的说法，并标记相互矛盾之处
- 从一个概念沿着双向链接快速发现相关人物、公司和事件
...
- 如果像本文一样是一个**综合知识库**（今天放一批访谈文稿，明天放论文、会议记录），就写成通用定位，让 AI 按主题自行组织
...
每条事实论断必须标注来源文件（`(source: 文件名.pdf)`）；**两个来源说法冲突时显式记录矛盾**；没有来源的论断标记为"待核实"。校验"同一事件多个版本是否自相矛盾"，靠的就是这一节。
...
### 第 4 步（可选）：安装 Obsidian Web Clipper 浏览器扩展

- 在 Chrome 应用商店添加 **Obsidian Web Clipper**（免费）
- 作用：把任何网页文章一键转换成 Markdown 文件
- 如果你的来源本来就是现成的 PDF，用不到它；但要把网页文章、公众号网页版等直接存进 `raw`，它非常顺手

### 第 5 步：投喂第一份文档

准备好第一份要投喂的资料。本文的演示是先放入一份文档（例如某场访谈/交流会的文字稿 PDF）。
...
>
> **另一个提示：** 如果来源是网传文稿等**未经证实的材料**，正好用得上 CLAUDE.md 的引用规则——每条论断标注来源文件、来源冲突时显式记录矛盾、无来源的论断标记"待核实"。LLM Wiki 天然适合管理这类可信度存疑的材料。
...
- 读取 PDF 全文
- 给出文稿摘要
- 列出**计划创建的所有 Wiki 页面**（按主题/概念拆分的观点页）
...
这是整套系统最有意思的地方。继续往 `raw` 里放入新来源——可以是同一主题的补充材料，也可以是**同一事件的另一个流传版本**（例如网传的《梁文锋投资者交流会》就有"文字稿"和"说话人整理版"等多个版本，都放进去）。
...
**关键观察点：** Claude 不是为新来源另起炉灶，而是在**更新已有的主题观点页**——把新来源中的对应内容逐条并入已有页面，并按 CLAUDE.md 的引用规则给每条观点标注来源文件。

更重要的是：当不同来源**说法冲突**时（数字对不上、表述含义不同、同一句话归属的说话人不同），Claude 会按规则在页面上**显式记录矛盾**并同时引用冲突双方——这正是校验"同一事件多个流传版本"是否自相矛盾的核心机制。

**这就是 Wiki 在履行它的职责**：知识不是堆积，而是被编织进已有的结构——来源之间的出入被系统性地暴露出来，而不是靠你逐段肉眼比对。

再看图谱视图：节点更多、连接更多。**每加一份来源，Wiki 就变得更聪明。**

### 第 9 步：提一个"跨来源"的问题

现在问一个需要同时用到多份来源信息的问题。比如在多版本文稿的场景下，可以这样问：

```
关于 华为给 DeepSeek 约1.6 万张卡 的说法，各个来源一致吗？核心观点是什么，
有没有只在其中一个来源出现的表述？
```
...
2. **提问**：直接问跨来源/跨版本的问题，AI 基于 Wiki 回答、引用页面，并指出版本间分歧
...
| **信息考据 / 传闻核查** | 投喂同一事件的多个流传版本（如网传某访谈的不同整理版），提炼观点、逐条标注来源、系统性暴露版本间矛盾 |
...
- [ ] （可选）安装 Obsidian Web Clipper 浏览器扩展
- [ ] 把第一份文档放入 `raw`
- [ ] 终端 `cd` 到 Vault 目录，启动 `claude`
- [ ] 发送摄取指令 → 确认计划 → `go ahead`
- [ ] 在 Obsidian 图谱视图查看成果
- [ ] 继续放入更多来源（含同一事件的其他版本），再次摄取，观察矛盾标记
